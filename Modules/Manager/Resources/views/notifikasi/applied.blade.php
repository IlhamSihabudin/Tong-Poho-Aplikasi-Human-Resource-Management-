                ...
                <div class="card-header white">
                    <div class="row align-items-center justify-content-between">
                        <div style="padding-left: 10px">
                            <h1 class="h1-responsive" style="font-size: 20pt;">All Submissions</h1>
                        </div>
                    </div>
                </div>
                        ...
                        <table id="dtMaterialDesignExample" class="table table-striped table-borderless" cellspacing="0" width="100%">
                            ...
                            @foreach($data_applied as $item)
                                <tr>
                                    ...
                                    <td>
                                        @if($item->confirm_status == "Approve")
                                            <span class="badge badge-success">{{ $item->confirm_status }}</span>
                                        @elseif($item->confirm_status == "Reject")
                                            <span class="badge badge-danger">{{ $item->confirm_status }}</span>
                                        @elseif($item->confirm_status == "Pending")
                                            <span class="badge badge-warning">{{ $item->confirm_status }}</span>
                                        @endif
                                    </td>
                                    <td>{{ $item->statement }}</td>
                                    <td>
                                        @if($item->confirm_status == "Approve" || $item->confirm_status == "Reject")
                                            <a class="btn-floating btn-default" disabled><i class="fa fa-ban"></i></a>
                                        @else
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <button type="button" onclick="document.location.href='{{ url('manager/notifikasi/approve/'.base64_encode($item->id_submission)) }}'" class="btn btn-success btn-rounded"><i class="fa fa-thumbs-o-up" style="font-size: 14pt" aria-hidden="true"></i></button>
                                                <button type="button" onclick="document.location.href='{{ url('manager/notifikasi/reject/'.base64_encode($item->id_submission)) }}'" class="btn btn-danger btn-rounded"><i class="fa fa-thumbs-o-down" style="font-size: 14pt" aria-hidden="true"></i></button>
                                            </div>
                                        @endif
                                    </td>
                                </tr>
